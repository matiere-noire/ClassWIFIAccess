...
function disableLesson(req, lesson, callback) {
  ...
  lessonToDB.updateDB(lesson.id, function(err) {
    if (err) {
      ...
      Classroom.findById(lesson.ClassroomId, { SchoolId: req.body.SchoolId }, null, function(err, classroom) {
        if (err) {
          callback(err)
        } else {
          Control.disableWiFi(classroom.DeviceId, req.body.SchoolId, lesson.id, function(err) {
            callback(err)
          })
        }
      })
